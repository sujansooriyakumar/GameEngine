...
ParticleEmitter::ParticleEmitter(int count_, std::string shaderName_, glm::vec3 position_)
{
	...
	for (int i = 0; i < particleCount; i++) {
		particles.push_back(new Particle(shaderProg, position, glm::vec3(1.0f, 1.0f, 0.0f), glm::linearRand(1.0f, 10.0f)));
	}
...
void ParticleEmitter::Update(float deltaTime_)
{
	for (int i = 0; i < particles.size(); i++) {
		...
		particles.at(i)->size = glm::linearRand(10.0f, 15.0f);
		particles.at(i)->colour = glm::vec3(glm::linearRand(0.0, 1.0), glm::linearRand(0.0, 1.0), glm::linearRand(0.0, 1.0));
		particles.at(i)->position += glm::vec3(glm::linearRand(0.0f, 0.2f), glm::linearRand(0.0f, 0.2f), glm::linearRand(0.0f, 0.2f)) * deltaTime_;
